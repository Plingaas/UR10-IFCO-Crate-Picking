    ...
    for result in results:
        if result.masks:
            ...
            for i, mask in enumerate(masks):
                ...
                for cnt in contours:
                    cv2.drawContours(mask_overlay, [cnt], -1, color.tolist(), thickness=cv2.FILLED)

            color_image = cv2.addWeighted(color_image, 0.7, mask_overlay, 0.3, 0.0)
    

    color_image = cv2.resize(color_image, (540, 960))
    depth_image = cv2.resize(cv2.rotate(depth_image, cv2.ROTATE_90_COUNTERCLOCKWISE), (540, 960))
