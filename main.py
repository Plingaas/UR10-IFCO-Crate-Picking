...
coordinate_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.5)  # Adjust size if needed
...
def ICP(pcd):
    ...
    crate_pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1, max_nn=30))
    icp_result = o3d.pipelines.registration.registration_icp(
        pcd, crate_pcd, threshold, trans_init,
...
try:
    ...
    while True:
        ...
        if CAMERA_TALL: complete_grayscale_mask = np.zeros((1920, 1080)).astype(np.uint8)
        else: complete_grayscale_mask = np.zeros((1080, 1920)).astype(np.uint8)

        complete_binary_mask = complete_grayscale_mask
        if masks_np is not None:
            for obj_idx, mask in enumerate(masks_np):
                mask = cv2.rotate(mask, cv2.ROTATE_90_CLOCKWISE)
                if not CAMERA_TALL: mask = cv2.resize(mask, (1080, 1920), interpolation=cv2.INTER_NEAREST)
                else: mask = cv2.resize(mask, (1920, 1080), interpolation=cv2.INTER_NEAREST)
        ...
        if poses.shape[0] == 3:
            ...
            if max_diff_x <= 7 and max_diff_y <= 7 and max_diff_z <= 7 and max_diff_yaw <= 0.1:
